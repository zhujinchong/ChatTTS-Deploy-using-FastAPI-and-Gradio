...
def tts(request: TTSInput):
    ...
    soundfile.write(io_buffer, audio_data, samplerate=24000, format="wav")
    io_buffer.seek(0)
...
def generate_tts_audio_streaming(request: TTSInput):
    # ========================参数=================================================
    # 选择speaker：https://modelscope.cn/studios/ttwwwaa/ChatTTS_Speaker
    rnd_spk_emb = chat.sample_random_speaker()
    # 推理参数
    ...
    for gen in wavs_gen:
        wavs = np.array([[]])
        wavs = np.hstack([wavs, np.array(gen[0])])
...
@app.post("/tts/stream")
async def tts_stream(request: TTSInput):
    tts_generator = generate_tts_audio_streaming(request)
